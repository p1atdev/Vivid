...
//
//  Updated by p1atdev on 2022/7/28
...
class LivePhoto {
    ...
    public class func generate(from imageURL: URL?, videoURL: URL, progress: @escaping (CGFloat) -> Void) async -> (PHLivePhoto?, LivePhotoResources?) {
        return await shared.generate(from: imageURL, videoURL: videoURL, progress: progress)
    }
    ...
    /// Save a Live Photo to the Photo Library by passing the paired image and video.
    public class func saveToLibrary(_ resources: LivePhotoResources) async -> Bool {
        do {
    ...
    private func generateKeyPhoto(from videoURL: URL) async -> URL? {
        ...
        if let stillImageTime = await videoAsset.stillImageTime() {
            guard let duration = try? await videoAsset.load(.duration).value else {
                return nil
            }
            
            percent = Float(stillImageTime.value) / Float(duration)
        }
        guard let imageFrame = try? await videoAsset.getAssetFrame(percent: percent) else { return nil }
        guard let jpegData = imageFrame.jpegData(compressionQuality: 1.0) else { return nil }
        guard let url = cacheDirectory?.appendingPathComponent(UUID().uuidString).appendingPathExtension("jpg") else { return nil }
        do {
            try? jpegData.write(to: url)
    ...
    private func generate(from imageURL: URL?, videoURL: URL,
                          progress: @escaping (CGFloat) -> Void) async -> (PHLivePhoto?, LivePhotoResources?) {
        guard let cacheDirectory = cacheDirectory else {
            return (nil, nil)
        }
        let assetIdentifier = UUID().uuidString
        let _keyPhotoURL: URL? = await { return (imageURL != nil) ? imageURL : await generateKeyPhoto(from: videoURL) }()
        guard let keyPhotoURL = _keyPhotoURL, let pairedImageURL = addAssetID(assetIdentifier, toImage: keyPhotoURL, saveTo: cacheDirectory.appendingPathComponent(assetIdentifier).appendingPathExtension("jpg")) else {
            return (nil, nil)
        }
        
        guard let pairedVideoURL = await addAssetID(assetIdentifier,
                   toVideo: videoURL,
        ...
        _ = PHLivePhoto.request(withResourceFileURLs: [pairedVideoURL, pairedImageURL],
                                placeholderImage: nil,
                                targetSize: CGSize.zero,
                                contentMode: PHImageContentMode.aspectFit,
                                resultHandler: { (_livePhoto: PHLivePhoto?, info: [AnyHashable : Any]) -> Void in
            if let isDegraded = info[PHLivePhotoInfoIsDegradedKey] as? Bool, isDegraded {
                return
            }
            livePhoto = _livePhoto
            print("PHLivePhoto.request", livePhoto)
        })
        
        print("generate", livePhoto)
        
        return (livePhoto, (pairedImageURL, pairedVideoURL))
    }
    ...
    private func saveAssetResource(_ resource: PHAssetResource, to directory: URL, resourceData: Data) -> URL? {
        let fileExtension = UTType(resource.uniformTypeIdentifier)
        ...
        fileUrl = fileUrl.appendingPathExtension(for: ext)
    ...
    func addAssetID(_ assetIdentifier: String,
                    toVideo videoURL: URL,
                    saveTo destinationURL: URL,
                    progress: @escaping (CGFloat) -> Void) async -> URL? {
        ...
        do {
            // Create the Asset Writer
            self.assetWriter = try AVAssetWriter(outputURL: destinationURL, fileType: .mov)
            // Create Video Reader Output
            self.videoReader = try AVAssetReader(asset: videoAsset)
            let videoReaderSettings = [kCVPixelBufferPixelFormatTypeKey as String: NSNumber(value: kCVPixelFormatType_32BGRA as UInt32)]
            let videoReaderOutput = AVAssetReaderTrackOutput(track: videoTrack, outputSettings: videoReaderSettings)
            self.videoReader?.add(videoReaderOutput)
            // Create Video Writer Input
            ...
            // Create Audio Reader Output & Writer Input
            videoAsset.loadTracks(withMediaType: .audio) { audioTracks, error in
                if let audioTrack = audioTracks?.first {
                    do {
                        let _audioReader = try AVAssetReader(asset: videoAsset)
                        let _audioReaderOutput = AVAssetReaderTrackOutput(track: audioTrack, outputSettings: nil)
                        _audioReader.add(_audioReaderOutput)
                        self.audioReader = _audioReader
                        audioReaderOutput = _audioReaderOutput
                        let _audioWriterInput = AVAssetWriterInput(mediaType: .audio, outputSettings: nil)
                        _audioWriterInput.expectsMediaDataInRealTime = false
                        self.assetWriter?.add(_audioWriterInput)
                        audioWriterInput = _audioWriterInput
                    } catch {
                        print(error)
                    }
            ...
            self.assetWriter?.metadata = [assetIdentifierMetadata]
            self.assetWriter?.add(stillImageTimeMetadataAdapter.assetWriterInput)
            // Start the Asset Writer
            self.assetWriter?.startWriting()
            self.assetWriter?.startSession(atSourceTime: CMTime.zero)
            // Add still image metadata
            let _stillImagePercent: Float = 0.5
            guard let timeRange = try? await videoAsset.makeStillImageTimeRange(percent: _stillImagePercent, inFrameCount: frameCount) else {
                return nil
            }
            stillImageTimeMetadataAdapter.append(AVTimedMetadataGroup(items: [self.metadataItemForStillImageTime()],
                                                                            timeRange: timeRange))
            // For end of writing / progress
            var writingVideoFinished = false
            var writingAudioFinished = false
            var currentFrameCount = 0
            func didCompleteWriting() async -> URL? {
                guard writingAudioFinished && writingVideoFinished else { return nil }
            ...
            // Start writing video
            if self.videoReader?.startReading() ?? false {
                ...
                if shouldComplete {
                    return await didCompleteWriting()
                }
            } else {
                writingVideoFinished = true
                return await didCompleteWriting()
            ...
            // Start writing audio
            if self.audioReader?.startReading() ?? false {
                var shouldComplete = false
                audioWriterInput?.requestMediaDataWhenReady(on: DispatchQueue(label: "audioWriterInputQueue")) {
                    while audioWriterInput?.isReadyForMoreMediaData ?? false {
                        guard let sampleBuffer = audioReaderOutput?.copyNextSampleBuffer() else {
...
fileprivate extension AVAsset {
    ...
    func countFrames(exact:Bool) async -> Int {
        ...
        if let videoReader = try? AVAssetReader(asset: self)  {
            ...
            if let videoTrack = try? await self.loadTracks(withMediaType: .video).first {
                
                guard let duration = try? await self.load(.duration),
                      let frameRate = try? await videoTrack.load(.nominalFrameRate) else {
                    return 0
                }
                
                frameCount = Int(CMTimeGetSeconds(duration) * Float64(frameRate))
    ...
    func stillImageTime() async -> CMTime?  {
        ...
        if let videoReader = try? AVAssetReader(asset: self)  {
            ...
            if let metadataTrack = try? await self.loadTracks(withMediaType: .metadata).first {
